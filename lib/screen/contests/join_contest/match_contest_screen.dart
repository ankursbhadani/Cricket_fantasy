...
class _Match_contest_screenState extends State<Match_contest_screen> with TickerProviderStateMixin {
  ...
  Future<void> get_points() async {
    int fantasyPoints = await match_points.get_fantasy_points(widget.match_id);
    // Here you might want to handle what to do with the fantasy points,
    // such as updating the UI or storing the points in the appropriate data structure.
    // For example, you can set the points in the state variable.
    setState(() {
      points['fantasyPoints'] = fantasyPoints;
    });
  }
